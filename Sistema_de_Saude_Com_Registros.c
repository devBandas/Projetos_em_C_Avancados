...
void salvarTudo() {
    ...
    // 1. Salvar Clientes
    arquivo = fopen("clientes.dat", "wb"); // wb = write binary (escrever binario)
    if (arquivo != NULL) {
        // Salva o array inteiro de uma vez
        fwrite(listaClientes, sizeof(Cliente), totalClientes, arquivo);
        fclose(arquivo);
    }

    // 2. Salvar Profissionais
    arquivo = fopen("profissionais.dat", "wb");
    if (arquivo != NULL) {
        fwrite(listaProfissionais, sizeof(Profissional), totalProfissionais, arquivo);
        fclose(arquivo);
    }

    // 3. Salvar Tipos de Profissao
    arquivo = fopen("profissoes.dat", "wb");
    if (arquivo != NULL) {
        fwrite(listaProfissoes, sizeof(Profissao), totalProfissoes, arquivo);
        fclose(arquivo);
    }

    // 4. Salvar Atendimentos
    arquivo = fopen("atendimentos.dat", "wb");
    if (arquivo != NULL) {
        fwrite(listaAtendimentos, sizeof(Atendimento), totalAtendimentos, arquivo);
        fclose(arquivo);
    }
...
void carregarTudo() {
    ...
    // 1. Carregar Clientes
    arquivo = fopen("clientes.dat", "rb"); // rb = read binary (ler binario)
    if (arquivo != NULL) {
        // O fread retorna quantos itens leu. Isso atualiza nosso contador!
        totalClientes = fread(listaClientes, sizeof(Cliente), MAX_CLIENTES, arquivo);
        fclose(arquivo);
    } else {
        totalClientes = 0; // Se o arquivo nao existe, comeca do zero
    }

    // 2. Carregar Profissionais
    arquivo = fopen("profissionais.dat", "rb");
    if (arquivo != NULL) {
        totalProfissionais = fread(listaProfissionais, sizeof(Profissional), MAX_PROFISSIONAIS, arquivo);
        fclose(arquivo);
    } else {
        totalProfissionais = 0;
    }

    // 3. Carregar Tipos de Profissao
    arquivo = fopen("profissoes.dat", "rb");
    if (arquivo != NULL) {
        totalProfissoes = fread(listaProfissoes, sizeof(Profissao), MAX_PROFISSOES, arquivo);
        fclose(arquivo);
    } else {
        totalProfissoes = 0;
    }
    ...
    if (arquivo != NULL) {
        totalAtendimentos = fread(listaAtendimentos, sizeof(Atendimento), MAX_ATENDIMENTOS, arquivo);
        fclose(arquivo);
    } else {
        totalAtendimentos = 0;
    }
...
int main(){
    // 1. CARREGA OS DADOS DO DISCO ASSIM QUE INICIA
    carregarTudo(); 
    ...
    do {
        ...
        switch(opcao){
            ...
            case 1: 
                cadastrarCliente();
                salvarTudo(); // Salva logo apos cadastrar!
                break;
            case 2:
                cadastrarProfissao();
                salvarTudo();
            ...
            // --- RELATORIOS (Nao precisa salvar, so ler) ---
            case 5:
                listarProfissionais(); 
                break;  
            case 6:
            ...
            // --- SAIDA ---
            case 0:
                salvarTudo(); // Salva uma ultima vez por seguranca
